...
class PrivateIngredientsApiTests(TestCase):
    ...
    def test_filter_ingredients_assigned_to_recipies(self):
        ...
        # Passing parameters "{'assigned_only': 1}"
        res = self.client.get(INGREDIENTS_URL, {'assigned_only': 1})

        s1 = IngredientSerializer(in1)
        s2 = IngredientSerializer(in2)
        self.assertIn(s1.data, res.data)
        # This should be the case because in2 isn't assigned to any recipe.
        self.assertNotIn(s2.data, res.data)

    def test_filtered_ingredients_unique(self):
        """Test filtering ingredients returns a unique list."""
        # To test if returned values are unique.
        ...
        res = self.client.get(INGREDIENTS_URL, {'assigned_only': 1})
        # Testing if the returned ingredients are unique
        self.assertEqual(len(res.data), 1)
